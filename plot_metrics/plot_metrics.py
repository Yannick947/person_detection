...
LOGGING_METRIC = 'epoch_mAP'
LOG_DIR_SUP = 'C:/Users/Yannick/Google Drive/person_detection/keras-retinanet/tensorboard_pcds/iou05'
LOG_DIRS = {'EfficientNetB2': 'best_effnetb2' , 
            'EfficientNetB3': 'best_effnetb3',
            'EfficientNetB7': 'best_effnetb7',
            'vgg16': 'best_vgg16',
            'resnet50': 'best_resnet50',
            'resnet152': 'best_resnet152'}
SMOOTH_NUM = 4
...
def plot_tensorflow_log(log_paths):
        ...
        for metric in event_acc.Tags()['scalars']: 
            ...
            if (metric in LOGGING_METRIC) and (metric not in parsed_metrics):               
                ...
                if ('validation' in path) or ('mAP' in metric): 
                    ...
                    plt.plot(x, y_smooth[:], label=f'validation {metric} {net.split()[0]}')
    ...
    plt.xlabel('epoch')
    plt.show()
    figure.savefig(f'pcds_bestvalidations_{LOGGING_METRIC}.png', type='png', dpi=2400)
    mpl.use("pgf")
    mpl.rcParams.update({
